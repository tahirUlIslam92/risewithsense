        ...
        {items.map((item) => (
          <div key={item.id} className="flex gap-3 p-4 bg-white rounded-2xl border border-[#EEE]">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Link href={`/products/${item.slug}`} className="text-sm font-medium hover:text-[#8B7355]">{item.name}</Link>
              <p className="text-sm font-bold mt-0.5">Rs. {item.price.toLocaleString()}</p>
              <div className="flex items-center gap-3 mt-2">
                <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 rounded-full border border-[#EEE] flex items-center justify-center hover:border-[#8B7355]">
                ...
                <button onClick={() => removeItem(item.id)} className="ml-auto flex items-center gap-1 text-xs text-red-500 uppercase">
