...
export default async function MyOrdersPage() {
  ...
  // Fetch orders by authenticated user ID
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      <main className="max-w-3xl mx-auto px-4 pt-24 pb-24 md:pt-28 md:pb-16">
        <h1 className="text-2xl font-bold text-[#1A1A1A] mb-8">My Orders</h1>
        ...
        {!orders || orders.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-[2rem] shadow-sm">
            ...
            <Link href="/products" className="inline-block px-6 py-3 bg-[#1A1A1A] text-white text-xs uppercase tracking-wider rounded-full hover:bg-[#8B7355] transition-colors">Browse Products</Link>
          </div>
        ) : (
          <div className="space-y-4">
            {orders.map((order: any) => {
              ...
              return (
                <div key={order.id} className="bg-white rounded-2xl border border-[#EEE] p-5 shadow-sm hover:shadow-md transition-all">
                    ...
                    {["pending", "confirmed", "shipped", "delivered"].map((s, i) => {
                      ...
                      return (
                        <div key={s} className="flex-1 flex items-center">
                          <div className={`h-1.5 rounded-full flex-1 transition-all ${isComplete ? "bg-[#8B7355]" : "bg-[#EEE]"}`} />
                          {i < 3 && <div className="w-1" />}
                        </div>
                      );
                    })}
                  </div>
                  <div className="flex justify-between text-[10px] text-[#999] uppercase mb-4">
                    <span>Pending</span><span>Confirmed</span><span>Shipped</span><span>Delivered</span>
                  </div>

                  <div className="flex justify-between items-center pt-4 border-t border-[#EEE]">
                    <p className="text-lg font-bold">Rs. {Number(order.total).toLocaleString()}</p>
